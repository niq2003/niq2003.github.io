var myApp = function(){
	...
	return {
		...
		timeGo:function(){//绘制指针 让指针走起来
			...
			this.setTimeTxt(numSet(hours)+":"+numSet(minutes)+":"+numSet(seconds),0,-115);
			this.setTimeTxt(numSet(month+1)+"月"+numSet(day)+"日",0,130);
			//绘制时针
			context.beginPath();
			...
			context.arc(0,-50,8,0,Math.PI*2,false);
			context.fill();
			context.stroke();
			//绘制秒针后面的圆圈
			context.beginPath();
			context.fillStyle = "RGB(224,120,49)";
			context.arc(0,15,6,0,Math.PI*2,false);
			context.fill();
			context.restore();//回到初始化的状态
			context.save();//保存初始化的状态;
